...
module.exports.run = async function ({ event, api, args }) {
    ...
    if (!mention) return api.sendMessage("sᴇɴsᴇɪ, ᴘʟᴇᴀsᴇ ᴍᴇɴᴛɪᴏɴ 1 ᴘᴇʀsᴏɴ.", threadID, messageID);
    else {
        var one = senderID, two = mention;
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "𝗦𝗵𝗶𝗽𝗽 𝘁𝗼𝗴𝗲𝘁𝗵𝗲𝗿 ",
            mentions: [{
          tag: tag,
          id: mention
        }],
