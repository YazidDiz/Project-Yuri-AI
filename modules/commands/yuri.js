module.exports.config = {
  ...
  version: "1.0.0",
  hasPermission: 0,
  description: "yuri bot that can download facebook videos, sing, search image, search lyrics and more..",
  credits: "Réynél",
  usages: "[help]",
  commandCategory: "ai",
  cooldowns: 10,
...
const { threadID, messageID, body } = event;
  const rand = ["Hi user if you don't know how to use me just type <yuri help>","Hey There user I'm Yuri Bot how can I assist you today? 😊", "Hello there user can I sing for you? 🥰", "Hello There user did you know that I can sing, download facebook videos, search image and more...", "Hello There user I'm Yuri Bot Created By Clark Shirosuzuka and the owner is Clark Shirosuzuka again :>."];
  const rand1 = rand[Math.floor(Math.random() * rand.length)];
  if (txt.toLowerCase() == "help"){
    const message = `•——[YURI BOT]——•
THESE ARE THE THINGS I CAN DO AS A BOT.

•YURI CAN SING SONGS.

Just type <yuri can you sing (title of song)> or <yuri sing (title of song)> as long as there is the word sing and the title of the song, it can automatically detect it


•YURI CAN DOWNLOAD FACEBOOK VIDEOS.

Just type <yuri can you download this facebook video (fb video url)> or <yuri download this facebook video (fb video url)> as long as there is the word ‘facebook video’ or ‘download’ and the fb video url, it can automatically detect it.


•YURI CAN SEARCH IMAGES.

Just type <yuri can you search an image of (your search)> or <yuri image of (your search)> as long as there is the word ‘image’ or ‘image of’ and your search, it can automatically detect it.


•YURI CAN SEARCH VIDEO FROM YOUTUBE.

Just type <yuri can you send a video of (your search)> or <yuri video of (your search)> and more... as long as there's the word ‘video of’ and you search, it will automatically send a video

•YURI CAN TALK.

Just type <yuri say (message)>.

•YURI CAN SEND A LYRICS.

Just type <yuri can you give me the lyrics of (song name)> or <yuri give me a lyrics of (song name)> as long as there's a word ‘lyrics of’ and the song name it will be automatically detected.

•YURI CAN DOWNLOAD TIKTOK VIDEOS USING LINK

Just type <yuri can you download this tiktok link (and tiktok your link)> or <yuri download this tiktok link (your tiktok link)>

•YURI CAN SEND A TIKTOK VIDEO

Just type <yuri send the tiktok video of (your search)> or <yuri send the tiktok video of (your search)> or <yuri send a tiktok video of (your search)>
...
if (!txt) return api.sendMessage(rand1, threadID, messageID);
 if (txt.toLowerCase().startsWith("thank you") || txt.toLowerCase().startsWith("thank")) return api.sendMessage("You're welcome user 🤗", threadID, messageID);
  ...
  if (txt.toLowerCase().includes("who created you") || txt.toLowerCase().includes("who's your master") || txt.toLowerCase().includes("who create you") || txt.toLowerCase().includes("who's your creator")) return api.sendMessage("I'm Yuri Shirosuzuka and my creator is Clark Shirosuzuka", event.threadID, event.messageID);
  if (txt.toLowerCase().includes("who is Clark?") || txt.toLowerCase().includes("who is Clark Shirosuzuka?")) return api.sendMessage("My Bot Admin", threadID, messageID);
