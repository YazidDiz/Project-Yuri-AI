...
module.exports.run = async function({ api, event, args, client, __GLOBAL, Users }) {
  ...
  if (args.join().indexOf('@') !== -1){ var uid = Object.keys(event.mentions) }*/
	...
	var text = args.join(" ");
	if (!text) return api.sendMessage(`❎ | 𝖲𝖾𝗇𝗌𝖾𝗂, 𝗍𝗁𝖺𝗍 𝗂𝗌 𝖺 𝗐𝗋𝗈𝗇𝗀 𝖿𝗈𝗋𝗆𝖺𝗍 𝗎𝗌𝖾 𝗈𝖿 𝗍𝗁𝖾 𝖼𝗈𝗆𝗆𝖺𝗇𝖽.\n🎓 | 𝗨𝘀𝗮𝗴𝗲𝘀: ${global.config.PREFIX}${this.config.name} 𝗍𝖾𝗑𝗍.`, threadID, messageID);
...
oms = await this.circle(avatar);
  ...
  ctx.font = "400 16px Arial";
	...
	return api.sendMessage({ body: "✅ | 𝖲𝖾𝗇𝗌𝖾𝗂, 𝗁𝖾𝗋𝖾’𝗌 𝗍𝗁𝖾 𝖼𝗈𝗆𝗆𝖾𝗇𝗍 𝖻𝗈𝖺𝗋𝖽:", attachment: fs.createReadStream(pathImg) }, threadID, () => fs.unlinkSync(pathImg), messageID);        
  }