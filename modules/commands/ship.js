module.exports.config = {
  ...
  hasPermssion: 0,
  credits: "Réynél",
  description: "ship to the person you tag",
...
module.exports.run = async function ({ event, api, args }) {
    ...
    if (!mention) return api.sendMessage("ℹ️ | 𝖲𝖾𝗇𝗌𝖾𝗂, 𝗄𝗂𝗇𝖽𝗅𝗒 𝗍𝖺𝗀 𝟣 𝗉𝖾𝗋𝗌𝗈𝗇", threadID, messageID);
    else {
        var one = senderID, two = mention;
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "𝗒𝗂𝖾 "  +  tag + ' 𝗌𝗁𝗂𝗉 𝗒𝖺𝗇 💔',
            mentions: [{
          tag: tag,
          id: mention
        }],
