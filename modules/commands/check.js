...
const fs = global.nodemodule["fs-extra"];
    ...
    if (args[0] == "all") {
        ...
        var page = 1;
            ...
            page < -1 ? page = 1 : "";
            var msg = "𝗥𝗮𝗻𝗸𝗶𝗻𝗴 𝗹𝗶𝘀𝘁:\n\n";
            var numPage = Math.ceil(exp.length/limit);
            for(var i = limit*(page - 1); i < limit*(page-1) + limit; i++){
                if(i >= exp.length) break;
                let dataInfo = exp[i];
                msg += `${i+1}: ${dataInfo.name}: ${dataInfo.exp} 𝗺𝗲𝘀𝘀𝗮𝗴𝗲𝘀\n`
            }       
            msg += `\n𝗣𝗮𝗴𝗲: ${page}/${numPage}`
        ...
        if (mention[0]) {
          ...
          const ZiaRein1 = (`𝗡𝗮𝗺𝗲: ${exp[rank].name}\n𝗥𝗮𝗻𝗸: ${rank + 1}\n𝗡𝘂𝗺𝗯𝗲𝗿 𝗼𝗳 𝗺𝗲𝘀𝘀𝗮𝗴𝗲𝘀: ${exp[rank].exp}\n𝗜𝗻𝘁𝗲𝗿𝗮𝗰𝘁𝗶𝗼𝗻 𝗿𝗮𝘁𝗲: ${(exp[rank].exp/count*100).toFixed(0)}%`);
          var link = [
...
"https://i.imgur.com/dpvjG2x.jpg",
    ...
    return request(encodeURI(link[Math.floor(Math.random() * link.length)])).pipe(fs.createWriteStream(__dirname + "/cache/ZiaReinC.jpg")).on("close", () => callback());
      ...
      const ZiaRein2 = (`𝗡𝗮𝗺𝗲: ${exp[rank].name}\n𝗥𝗮𝗻𝗸: ${rank + 1}\n𝗡𝘂𝗺𝗯𝗲𝗿 𝗼𝗳 𝗺𝗲𝘀𝘀𝗮𝗴𝗲𝘀: ${exp[rank].exp}\n𝗜𝗻𝘁𝗲𝗿𝗮𝗰𝘁𝗶𝗼𝗻 𝗿𝗮𝘁𝗲: ${(exp[rank].exp/count*100).toFixed(0)}%`);
