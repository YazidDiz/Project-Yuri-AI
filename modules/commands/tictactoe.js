	...
	for (var i = 0; i < 3; i++) {
		for (var j = 0; j < 3; j++) {
			...
			if (temp == "0") msg += "[  ]  "
				...
				(data.isX) ? msg += "[𝗢]  " : msg += "[𝗫]  ";
			}   
			else {
				(data.isX) ? msg += "[𝗫]  " : msg += "[𝗢]  ";
			}
		}
		msg += "\n";
	}
	...
	if(body && /^[0-9]*$/.test(body)) {
		...
		if(row >= 0 && row <= 2 && col >= 0 && col <= 2) {
			...
			if(checkGameOver(data)) {
				if(checkAIWon(data)) api.sendMessage("👾 | 𝖲𝖾𝗇𝗌𝖾𝗂, 𝖨 𝗐𝗂𝗇, 𝖻𝖾𝗍𝗍𝖾𝗋 𝗅𝗎𝖼𝗄 𝗇𝖾𝗑𝗍 𝗍𝗂𝗆𝖾.", threadID, messageID);
				else if(checkPlayerWon(data)) api.sendMessage("🎉 | 𝖢𝗈𝗇𝗀𝗋𝖺𝗍𝗎𝗅𝖺𝗍𝗂𝗈𝗇𝗌 𝗌𝖾𝗇𝗌𝖾𝗂, 𝗒𝗈𝗎 𝗐𝗂𝗇!", threadID, messageID);
				else api.sendMessage("👾 | 𝖲𝖾𝗇𝗌𝖾𝗂, 𝗂𝗍'𝗌 𝖺 𝖽𝗋𝖺𝗐, 𝗅𝖾𝗍'𝗌 𝗉𝗅𝖺𝗒 𝖺𝗀𝖺𝗂𝗇?", threadID, messageID);
	   ...
	   switch (args[0]) {
		   case "random": 
				if(Math.random() > 0.5) {
				  ...
				  api.sendMessage("ℹ️ | 𝖲𝖾𝗇𝗌𝖾𝗂, 𝗒𝗈𝗎 𝗀𝗈 𝖿𝗂𝗋𝗌𝗍, 𝗫", threadID, messageID);
				  api.sendMessage(`${displayBoard(newData)}`, threadID, messageID);
				}
				else {
					newData = startBoard({isX: false, data, threadID});
					AIStart(newData);
					api.sendMessage("ℹ️ | 𝖲𝖾𝗇𝗌𝖾𝗂, 𝖨 𝗀𝗈 𝖿𝗂𝗋𝗌𝗍, 𝗢", threadID, messageID);
				}
			break;
			case "x":
				newData = startBoard({isX: true, data, threadID});
