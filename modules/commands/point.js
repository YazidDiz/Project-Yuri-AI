...
module.exports.run = async function ({ event, api, args, client }) {
    ...
    if (!mention) return api.sendMessage("sᴇɴsᴇɪ, ᴘʟᴇᴀsᴇ ᴍᴇɴᴛɪᴏɴ 1 ᴘᴇʀsᴏɴ", event.threadID, event.messageID);
    else {
        var one  = mention;
        return makeImage({ one }).then(path => api.sendMessage({ body: event.mentions[mention].replace("@", "") + " 𝖮𝗆𝖺𝗂𝗋𝖺𝖺𝖺!!!", attachment: fs.createReadStream(path) }, threadID, () => fs.unlinkSync(path), messageID));
    }
}
