...
module.exports.run = async function ({ api, event, args }) {
	...
	if (!content) return out("ℹ️ | 𝖲𝖾𝗇𝗌𝖾𝗂, 𝗉𝗅𝖾𝖺𝗌𝖾 𝖾𝗇𝗍𝖾𝗋 𝗍𝗁𝖾 𝖼𝖺𝗅𝖼𝗎𝗅𝖺𝗍𝗂𝗈𝗇.");
		...
		try {
			...
			var img = (await axios.get(src, { responseType: 'stream' })).data;
			img.pipe(fs.createWriteStream("./graph.png")).on("close", () => api.sendMessage({ body: (!result) ? '' : result + "\n𝗩𝗲𝗰𝘁𝗼𝗿 𝗟𝗲𝗻𝗴𝘁𝗵: " + vector_length, attachment: fs.createReadStream("./graph.png") }, threadID, () => fs.unlinkSync("./graph.png"), messageID));
		}
		catch (e) {
			out(`${e}`);
		}
