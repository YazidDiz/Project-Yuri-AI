        ...
        var random2 = job[Math.floor(Math.random() * job.length)]
                ...
                for (singleData of threadIn4.userInfo) {
                    ...
                    try {
                        ...
                        logger(global.getText('handleCreateDatabase', 'newUser', chalk.hex("#" + random)(`New user:  `) + chalk.hex("#" + random1)(`${singleData.name}`) + "  ||  " + chalk.hex("#" + random2)(`${singleData.id}`)), '《 YURI USER 》'));
                    } catch(e) { console.log(e) };
                }
                logger(global.getText('handleCreateDatabase', 'newThread',chalk.hex("#" + random)(`New group: `) + chalk.hex("#" + random1)(`${threadID}`) + "  ||  " + chalk.hex("#" + random2)(`${threadIn4.threadName}`)), '《 YURI THREAD 》');
            }
            if (!allUserID.includes(senderID) || !userName.has(senderID)) {
                ...
                logger(global.getText('handleCreateDatabase', 'newUser', chalk.hex("#" + random)(`New users: `) + chalk.hex("#" + random1)(`${singleData.name}`) + " || " + chalk.hex("#" + random2)(`${senderID}`)), '《 YURI USER 》');
            }
